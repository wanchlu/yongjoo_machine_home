...
I should also add, the statement about "thoroughly tested" above is mostly a joke. This is the 
first version of the OO interface, and it has I<barely> been tested at all, so if something 
...
sub _img_ptr_from_file
{
    ...
    return gdImageCreateFromWBMP( $filename )
        if( $type eq 'wbmp' );
        
    return gdImageCreateFromGd( $filename )
        if( $type eq 'gd' );
    
    return gdImageCreateFromGd2( $filename )
        if( $type eq 'gd2' );
    
    return gdImageCreateFromGif( $filename )
        if( $type eq 'gif' );
            
    return gdImageCreateFromXbm( $filename )
        if( $type eq 'xbm' );
...
sub new
{
    ...
    {   # Create it from a PDL variable:
        ...
        {
            die "Can't create a PDL::IO::GDImage from a PDL with bad dims!\n";
        }
    }
...
sub Destroy
{
    return gdImageDestroy ( @_ );
} # End of Destroy()...


=head2 gdImageDestroy
...
{
    my $self = shift;
    return _gdImageDestroy ( $self->{IMG_PTR}, @_ );
} # End of gdImageDestroy()...
