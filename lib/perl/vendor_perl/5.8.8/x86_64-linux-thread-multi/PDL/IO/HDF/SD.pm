...
sub new
{
    ...
    {   # Create new file
        $filename = substr ($filename, 1);      # chop off -
        print "$sub: Creating HDF File $self->{FILE_NAME}\n"
            if $debug;
    ...
    foreach my $i ( 0 .. $num_datasets-1 )
    {
        ...
        foreach my $j ( 0 .. $self->{DATASET}->{$name}->{RANK}-1 )
        {
            ...
            $self->{DATASET}->{$name}->{DIMS}->{$dimname}->{SIZE} = $size;
            $self->{DATASET}->{$name}->{DIMS}->{$dimname}->{DIMID} = $dim_id;
            ...
            unless ( $size )
            {
                $self->{DATASET}->{$name}->{DIMS}->{$dimname}->{REAL_SIZE} = _SDgetunlimiteddim( $sds_id, $j);
            }
        }
...
{
    my $self = shift;
    return $self->SDgetattributename( @_ );
...
sub SDgetdimsize
{
    ...
    # FIXME: This is a great candidate for overhaul! The data structure should be self serving,
    #    and the code style in this portion is no good (unclear).
    # NOTE: This should be done in new, and simply stored in an array and returned here.
    my @dims;
    foreach( keys %{ $self->{DATASET}->{$name}->{DIMS} } )
    {
        $dims[ $self->{DATASET}->{$name}->{DIMS}->{$_}->{NUMBER} ] 
...
sub SDgetunlimiteddimsize
{
    ...
    foreach( keys %{$self->{DATASET}{$name}{DIMS}} )
    {
        ...
        {
            $dim[ $self->{DATASET}->{$name}->{DIMS}->{$_}->{NUMBER} ] = 
...
sub SDget
{
    ...
    {   # \@end was not passed in, so we need to set everything else to defaults:
        ($start, $end) = [];
        foreach my $dim ( keys %{ $self->{DATASET}->{$name}->{DIMS} } )
        {
            my $use_size = $self->{DATASET}->{$name}->{DIMS}->{$dim}->{SIZE} 
                || $self->{DATASET}->{$name}->{DIMS}->{$dim}->{REAL_SIZE};

            $$end[ $self->{DATASET}->{$name}->{DIMS}->{$dim}->{NUMBER} ] = $use_size;            
            $$start[ $self->{DATASET}->{$name}->{DIMS}->{$dim}->{NUMBER} ] = 0;
            $$stride[ $self->{DATASET}->{$name}->{DIMS}->{$dim}->{NUMBER} ] = 1;
        }
    }
...
sub SDsetdimname
{
    ...
    {
        return( undef )
            unless defined( $$dimname[ $self->{DATASET}->{$name}->{DIMS}->{$_}->{NUMBER} ] );
        
        $res = _SDsetdimname( 
            $self->{DATASET}->{$name}->{DIMS}->{$_}->{DIMID}, 
            $$dimname[ $self->{DATASET}->{$name}->{DIMS}->{$_}->{NUMBER} ] ) + 1;
    }
    return( $res );
} # End of SDsetdimname()...
