...
of Perl and C, so that they are reliably present even when there no
...
MatrixOps includes algorithms and pre-existing code from several origins.
In particular, C<eigens_sym> is the work of Stephen Moshier, C<svd> 
uses an SVD subroutine written by Bryant Marks, and C<eigens> uses a 
subset of the Small Scientific Library by Kenneth Geisshirt.  All are
...
PLEASE NOTE: THE EIGENS FUNCTION HAS BEEN DISABLED FOR ASYMMETRIC MATRICES
PENDING FIX TO THE CODE.

The eigens function will attempt to compute the eigenvalues and eigenvectors
of a square matrix with real components.  If the matrix is symmetric, the
same underlying code as L<"eigens_sym"|/eigens_sym> is used.  If asymmetric,
the eigenvalues and eigenvectors are computed with algorithms from the
sslib library (anyone know what algorithm is used?).  If any imaginary
components exist in the eigenvalues, the results are currently considered
to be invalid, and such eigenvalues are returned as "NaN"s.  This is true
for eigenvectors also.  That is if there are imaginary components to any
of the values in the eigenvector, the eigenvalue and corresponding eigenvectors
are all set to "NaN".  Finally, if there are any repeated eigenvectors, they
are replaced with all "NaN"s.
...
If your assymmetric matrix returns all NaNs, do not assume that the values
are complex.  Also, problems with memory access is known in this library.
...
=cut
   ...
   sub PDL::eigens {
      ...
      if ( $deviation <= 1e-5 ) {
          ...
          barf "eigens currently disabled for asymmatric matrices\n";
