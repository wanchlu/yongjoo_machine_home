#!/usr/bin/perl

use strict;

my %feathash;
my $maxfeat = 0;

my $trainFile = shift;
my $trainsize = shift;
my $testFile = shift;
my $testsize = shift;
my $featureNameFile = "feature_names.txt";
open (FEATURENAME, ">$featureNameFile");

`./shuffle.pl $trainFile | head -$trainsize > $trainFile.shuffle`;
`./shuffle.pl $testFile | head -$testsize > $testFile.shuffle`;

open (TRAIN, "$trainFile.shuffle");
open (TRAINLABELS, ">$trainFile.labels");
open (NEWTRAIN, ">$trainFile.edges");

my $count = 0;

while (<TRAIN>) {
  chomp;
  $count++;
  my @feats = split (/\s+/);
  print TRAINLABELS $feats[0], "\n";
  foreach my $f (1..$#feats) {
	next if $f == 5 or $f == 9 or $f == 10 or $f == 11;
    print NEWTRAIN hashvalue($feats[$f]), " ", $count, " 1\n";
  }
}

close TRAIN;
close TRAINLABELS;
close NEWTRAIN;

open (TEST, "$testFile.shuffle");
open (TESTLABELS, ">$testFile.labels");
open (NEWTEST, ">$testFile.edges");

my $count = 0;

while (<TEST>) {
  chomp;
  $count++;
  my @feats = split (/\s+/);
  print TESTLABELS $feats[0], "\n";
  foreach my $f (1..$#feats) {
    print NEWTEST hashvalue($feats[$f]), " ", $count, " 1\n";
  }
}

close TEST;
close TESTLABELS;
close NEWTEST;


sub hashvalue {
  my $f = shift;
  if (exists $feathash{$f}) {
    return $feathash{$f};
  }
  else {
    $feathash{$f} = ++$maxfeat;
    print FEATURENAME "$f\n";
    return $maxfeat;
  }
}
