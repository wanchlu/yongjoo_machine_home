...
foreach my $it (1..$iterations) {
    ...
    print `java -cp ./HTAL/bin wanchen.HTAL ./data/java_input1.txt ./data/java_input2.txt ./data/java_input3.txt $al_size`;
    ...
    foreach my $round (1..$rounds) {
        ...
        my $f1 = `java -cp ./HTAL/bin wanchen.HTAL ./data/java_input1.txt ./data/java_input2.txt ./data/java_input3.txt 0`;
        print $f1;
        chomp ($f1);
        $f1 =~ s/.*Accuracy:\s*//g;
        $s1 += $f1;
        if ($round == 0 and $it == 0) {
            ...
            `echo $f1 > /home/wanchen/work/tumbl-clean/hitting_time_$seed_size_for_normal-$test_size_for_normal.txt`;
        }
    }
