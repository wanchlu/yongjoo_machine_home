...
unsigned int n_edges=0, n_nodes=0, n_labeled=0;

short n_classes;

char *train_label_file, *test_label_file, *sim_file, *output_file;

double *class_probs, *class_masses;
...
int main (int argc, char **argv) {
  ...
  class_probs = (double *) calloc (n_classes , sizeof (double));
  class_masses = (double *) calloc (n_classes , sizeof (double));
...
void normalize_class_probs () {
  ...
  for (short i=0; i < n_classes; i++) {
    fprintf (stderr, "class prob %d:  %f\n", i, class_probs[i]);
  }

  for (unsigned int i=0; i < n_nodes; i++) {
    if (!nodes[i].labeled)
      for (int j=0; j < n_classes; j++)
        class_masses[j] += nodes[i].old_values[j];
  }

  for (short i=0; i < n_classes; i++) {
    fprintf (stderr, "class mass %d:  %f\n", i, class_masses[i]);
  }

  for (unsigned int i=0; i < n_nodes; i++) {
    if (!nodes[i].labeled)
      for (int j=0; j < n_classes; j++)
        nodes[i].old_values[j] *= class_probs[j] / class_masses[j];
  }
}
