...
open OUT, ">inter/indexes_of_arguing_lexicon_feature.txt" or die;
my $idx = 1;
foreach (@ranked_f) {
    $f_index{"$_"} = $idx;
    print OUT $_."\t".$idx."\n";
...
open OUT, ">inter/arguing_lexicon_feature.libsvm" or die;
...
foreach my $domain (@domains) {
    ...
    my $file = "./libsvm/$domain/arguing_lexicon_feature.libsvm";
    `grep $domain "./inter/arguing_lexicon_feature.libsvm" | cut -f2 | grep "^1" > temp1`;
    `grep $domain "./inter/arguing_lexicon_feature.libsvm" | cut -f2 | grep "^-1" > temp2`;
    my $size = `wc -l temp1`;
    chomp ($size);
    $size =~ s/(\d+)\s+.*/$1/g;
    my $size2 = `wc -l temp2`;
